...
var Trips = (function () {
  ...
  /* 収益版モックアップだけの導線。いま開いている旅行の目的地・日程を
     そのままクエリに乗せて予約サイト（偽URL）へ渡す。行程を組み立てる前の
     「宿・航空券を決める」段階で使われることを想定し、旅行タブに置く。 */
  function travelAdUrl(trip) {
    return 'https://travel-affiliate.example/search?dest=' +
  ...
  function travelAdCard(trip) {
    if (!trip) return '';
    return '<a class="ad-cta" href="' + UI.esc(travelAdUrl(trip)) +
      '" target="_blank" rel="noopener">宿・航空券を探す' +
      '<span class="ad-pr-tag" style="margin-left:8px">PR</span></a>';
  }
